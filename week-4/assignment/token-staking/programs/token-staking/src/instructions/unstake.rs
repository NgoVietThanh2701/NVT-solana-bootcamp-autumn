...
pub struct Unstake<'info> {
    ...
    #[account(
        mut,
        close = staker,
        has_one = staker, // check stake_info.staker = staker
        has_one = mint,   // check stake_info.mint = mint
...
pub fn unstake(ctx: Context<Unstake>) -> Result<()> {
    ...
    let reward = stake_amount
        .checked_mul(slot_passed).unwrap()
        .checked_div(100).unwrap();

    msg!("slot passed: {}", slot_passed);
    msg!("reward: {}", reward);

    // transfer reward to staker
    let reward_vault_bump = ctx.bumps.reward_vault;
    let mint_key = ctx.accounts.mint.key();
    let reward_vault_signer_seeds: &[&[&[u8]]] = &[&[REWARD_VAULT_SEED, mint_key.as_ref() , &[reward_vault_bump]]];
    transfer(
    ...
    // transfer token from vault to token staker account
